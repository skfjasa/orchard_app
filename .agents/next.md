...
Retest Fruit tab fixes, fixture chat simulation, read-state persistence, new-match highlight persistence, and `/onboarding` background sizing.
...
- Fruit tab includes real/dev backend profiles and static Fruit fixtures.
- Fruit fixture likes auto-match for testing.
- One-sided Fruit/profile-detail likes use in-app UI, not browser alerts.
- Read Inbox messages stay read after sign-out/sign-in until newer messages arrive.
...
1. Open Fruit and verify both static Fruit fixtures and real/dev backend profiles can appear.
2. Tap a Fruit fixture heart and verify an in-app match overlay appears.
3. Tap a real/dev Fruit heart for a one-sided like and verify an in-app "Like sent" overlay appears.
4. Open an existing fixture conversation, send a message, and verify a fake fixture reply appears.
5. Read an unread real backend conversation, sign out, sign back in, and verify it stays read unless a newer message arrived.
6. Open a highlighted new match profile, sign out, sign back in, and verify that match stays unhighlighted.
7. Sign in as `t`, send messages to `tt` and `test2`, sign out, then sign in as `tt` and `test2` and verify Matches/Inbox hydrate without making a swipe.
