...
- Backend real/dev match cards now have a fallback render path if remembered profile objects are briefly unavailable, and Matches/Inbox trigger backend match/profile refresh on focus. Match detail receives an explicit `from` origin from Matches, Inbox, Discover, Fruit, and Chat, then uses that for canonical Android hardware-back behavior instead of relying only on ambiguous stack history.
...
1. Smoke-UAT device/browser back from match detail and chat: real/dev matches should remain visible after returning from profile detail, detail should not bounce back onto itself, Inbox should remain stable after rapid back from Chat, and Fruit/Discover should not be required to restore backend profile cards.
