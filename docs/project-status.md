...
- Backend match/thread hydration application calculations now live behind `expo/services/backend-match-hydration-application-service.ts`. `ProfileProvider` still owns the React state setters, but liked/new-match/conversation merge calculations are service-owned.
...
2. Continue provider-internal cleanup after Slice 6/Slice 5b: isolate remaining backend match hydration application and backend-first action paths without changing visible UI.
