...
- Foundation Slice 3 moves local read/seen preferences into `expo/store/use-preferences-store.ts` while preserving existing storage keys and provider wrappers. `ProfileProvider.markRead` and `ProfileProvider.markMatchSeen` remain screen-facing compatibility APIs; local read-watermark and seen-match preference application are now pure helpers in `expo/services/local-interaction-service.ts`, and Supabase `match_read_states` writes remain coordinated through the provider/backend chat service boundary.
...
- Pure backend conversation merge/read-through helpers now live in `expo/services/local-interaction-service.ts`; `ProfileProvider` still owns backend chat hydration orchestration.
...
- Local match activation, pass-state mutation, and stale-match cleanup now live behind `expo/services/local-match-action-service.ts`. `ProfileProvider` still delegates backend pass persistence and local match removal for unmatch/backend chat `match_not_found` cleanup.
...
- Local profile mutation persistence application now lives behind `expo/services/local-profile-mutation-service.ts`. `ProfileProvider` still owns the public profile/partner action wrappers and backend profile update side effect.
...
- Backend chat send/read action orchestration now lives behind `expo/services/backend-chat-action-service.ts`. `ProfileProvider` still owns local visible backend-message merge updates when backend sends succeed.
...
- Backend match/thread hydration planning now lives behind `expo/services/backend-match-hydration-service.ts`. `ProfileProvider` still applies the returned plan into local compatibility state, but backend profile repair, thread loading, and partial-hydration detection are service-owned.
- Backend match/thread hydration application now lives behind `expo/services/backend-match-hydration-application-service.ts`. `ProfileProvider` still owns readiness flags and hydration run timing, but ready-plan application into active match ids, liked ids, new-match ids, and conversations is service-owned.
- Backend profile complete-onboarding and update calls now live behind `expo/services/backend-profile-action-service.ts`. `ProfileProvider` still applies the resulting local profile state and persistence mutation.
- Backend profile bootstrap loading and pending onboarding recovery now live behind `expo/services/backend-profile-bootstrap-service.ts`. `ProfileProvider` still applies the loaded profile and backend hydration flag, but backend profile load, pending draft lookup, pending profile save, cancellation checks, and pending storage cleanup are service-owned.
- Backend swipe persistence and visible-match decisions now live behind `expo/services/backend-swipe-action-service.ts`. `ProfileProvider` still owns local fixture activation, match-limit checks, and super-like counters.
- Privacy/logging audit on 2026-07-08 confirmed there are no production analytics calls wired. The analytics service exists only as an unused interface/mock adapter, and current chat/profile diagnostics log ids, counts, lengths, and status/error metadata rather than private message bodies, raw profile text, or PII. The broader analytics/crash reporting stack decision remains open.
- Fixture content audit on 2026-07-08 confirmed seeded/local demo rows use deterministic fixture emails/ids and fictional mock copy. The original human-like dating/profile language is intentionally preserved for simulation realism; no real user data was identified.
- Interim Supabase Studio moderation workflow is documented in `docs/supabase-moderation-workflow.md` for inner-circle testing. It covers report review, account deletion request review, block/unmatch verification, manual visibility/suspension actions, and current limitations until an admin surface exists.
- A repeatable full-flow UAT checklist now lives in `docs/milestone-tracker.md`, covering auth/onboarding/profile/photos, session bootstrap/navigation, discovery/swipes/matches, chat/read state, safety/moderation, mock-mode regression, and result recording.
- Root Expo Router error boundary now exists in `expo/app/_layout.tsx` with a retry fallback and message-only diagnostic logging, giving broader beta testing a controlled app-level crash fallback without adding analytics/crash reporting yet.
- Source audit confirmed Supabase RLS/tests already cover invisible/suspended/incomplete/unverified discovery exclusion, blocked-profile discovery exclusion in both directions, suspended actor swipe denial, and blocked match message read/send denial. The milestone tracker now treats the code/RLS side as done and leaves hosted real-app UAT as the remaining acceptance step.
- `expo/eas.json` now exists with internal preview and production build profiles plus an iOS submit placeholder. Apple Developer/App Store Connect access and human verification of app metadata remain required before TestFlight.
- Report, block, and account-deletion safety service calls now live behind `expo/services/safety-action-service.ts`. `ProfileProvider` still owns the profile-required guard, local block cleanup, and account-deletion sign-out.
- Profile-provider sign-out reset bookkeeping now lives behind `expo/services/profile-provider-reset-service.ts`. Supabase auth sign-out still happens before local reset, and `ProfileProvider` still persists the cleared profile.
- Backend bootstrap reset bookkeeping for mock mode, missing-user state, and account switches now lives behind `expo/services/profile-provider-reset-service.ts`. The provider still decides which reset path applies from auth mode/session state.
