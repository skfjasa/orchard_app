...
- Backend chat send/read action orchestration now lives behind `expo/services/backend-chat-action-service.ts`. `ProfileProvider` still owns local visible conversation updates, stale local-match cleanup when the backend has no active match, and local simulated reply/photo mutation callbacks.
...
2. Continue provider-internal cleanup after Slice 6/Slice 5b: move the next small state domain, likely remaining local simulated conversation mutation callbacks or fixture match repair paths, out of `ProfileProvider` behind clearer services without changing visible UI.
