...
- Foundation Slice 6 has started with the Matches read path: `expo/hooks/use-matches-read-model.ts` now wraps the Matches tab's compatibility facade reads and focus refresh, and `expo/app/(tabs)/matches.tsx` no longer imports `useProfile()` directly.
...
Foundation Slice 6 step 1 implemented: Matches read path now uses `useMatchesReadModel`, preserving the existing UI, focus refresh, transient-empty guard, new-match highlighting, and seen-before-navigation behavior.
...
2. Continue foundation Slice 6 with the Inbox read path, migrating it from direct `useProfile()` usage to a focused hook/selectors without changing visible UI.
