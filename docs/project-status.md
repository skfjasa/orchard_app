...
- Foundation Slice 6 has started with route read-model migration. `expo/hooks/use-matches-read-model.ts` now wraps the Matches tab's compatibility facade reads and focus refresh, and `expo/app/(tabs)/matches.tsx` no longer imports `useProfile()` directly. `expo/hooks/use-inbox-read-model.ts` now wraps Inbox rows, typing state, shared-couple display, focus refresh, and `markRead`, and `expo/app/(tabs)/inbox.tsx` no longer imports `useProfile()` directly. `expo/hooks/use-match-detail-read-model.ts` now wraps Match Detail profile lookup, active-match state, super-like state, seen-state calls, and existing action wrappers, and `expo/app/match/[id].tsx` no longer imports `useProfile()` directly. `expo/hooks/use-chat-thread-read-model.ts` now wraps Chat profile/thread lookup, active-match guard state, typing/draft state, read marking, local send/delete/photo actions, and safety actions, and `expo/app/chat/[id].tsx` no longer imports `useProfile()` directly. `expo/hooks/use-discover-read-model.ts` now wraps the Discover deck's discovery query, remembered display profiles, exclusions, and swipe actions. `expo/hooks/use-fruit-read-model.ts` now wraps Fruit's backend non-fixture discovery plus local fixture pool/scoring behavior. `expo/hooks/use-edit-profile-read-model.ts`, `use-paywall-read-model.ts`, `use-report-read-model.ts`, and `use-safety-legal-read-model.ts` now wrap the corresponding route provider calls.
...
- Session-close handoff records the UAT tunnel, navigation back, forgot-password recovery checkpoint, and provider extraction checkpoint. Latest implementation checkpoint: pending commit - Profile/safety/paywall route facades extracted.

## Current Task

Foundation Slice 6 read-path migration now covers Matches, Inbox, Match Detail, Chat, Discover, Fruit, Edit Profile, Paywall, Report, and Safety & Legal. The latest low-risk route facade slice preserves existing edit-profile save behavior, demo paywall actions, report submission, and account-deletion request behavior.
...
2. Continue foundation Slice 6 with Profile tab, onboarding photos/sign-in, tab badges, root redirect, and ProtectedRoute/bootstrap gates, keeping bootstrap/auth behavior unchanged.
