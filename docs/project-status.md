...
- Session-close handoff records the UAT tunnel, navigation back, forgot-password recovery checkpoint, and provider extraction checkpoint. Latest implementation checkpoint: pending commit - Chat thread read model extracted.

## Current Task

Foundation Slice 6 read-path migration now covers Matches, Inbox, Match Detail, and Chat. Chat uses `useChatThreadReadModel` while preserving existing navigation/back behavior, active-match guard, read-marking effect, draft handling, text/photo send actions, long-press message actions, and safety menu behavior.
...
2. Continue foundation Slice 6 with Discover/Fruit discovery and local fixture behavior, without changing visible UI or mock/demo availability.
