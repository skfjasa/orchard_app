...
- Session-close handoff records the UAT tunnel, navigation back, forgot-password recovery checkpoint, and provider extraction checkpoint. Latest implementation checkpoint: pending commit - Extract provider selector helpers.

## Current Task

Provider-internal cleanup has started after Slice 6. The current slice extracts pure compatibility selector calculations from `ProfileProvider` while preserving route hooks, facade members, transient-empty behavior, and profile cache repair.
...
2. Continue provider-internal cleanup after Slice 6: move the next small state domain out of `ProfileProvider` behind clearer stores/services without changing visible UI.
