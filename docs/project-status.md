...
- Backend real/dev match cards no longer use a generic fallback profile in the common lookup path. Backend match hydration now preserves real remembered profile objects when available, queues a follow-up refresh if Matches/Inbox focus or realtime refresh lands while another refresh is in flight, and Matches/Inbox trigger backend match/profile refresh on focus. Match detail receives an explicit `from` origin from Matches, Inbox, Discover, Fruit, and Chat, then uses that for canonical Android hardware-back behavior instead of relying only on ambiguous stack history.
