...
- Session-close handoff records the UAT tunnel, navigation back, forgot-password recovery checkpoint, and provider extraction checkpoint. Latest implementation checkpoint: pending commit - Complete route read-model migration.

## Current Task

Foundation Slice 6 route read-model migration now covers all current app route/component `useProfile()` consumers. Direct provider access is confined to focused hooks, while route UI, auth/bootstrap gates, tab badges, onboarding finish, sign-in readiness, Profile tab behavior, and safety/paywall/report/edit flows are preserved.
...
2. Continue provider-internal cleanup after Slice 6: shrink `ProfileProvider` ownership of compatibility selectors and move remaining domain state behind clearer stores/services without changing visible UI.
