...
- Backend display-profile completeness and selection helpers now live behind `expo/services/backend-profile-display-service.ts`, shared by profile-provider lookup/cache hydration and backend match hydration planning.
...
2. Continue provider-internal cleanup after Slice 6/Slice 5b: isolate remaining backend-first action and display-profile cache ownership paths without changing visible UI.
