...
- Session-close handoff records the UAT tunnel, navigation back, forgot-password recovery checkpoint, and provider extraction checkpoint. Latest implementation checkpoint: `646104c` - Extract conversation merge helpers.

## Current Task

Provider-internal cleanup after Slice 6 has moved pure compatibility selectors, prototype monetization state, local chat UI state, and pure backend conversation merge/read-through helpers out of `ProfileProvider` while preserving route hooks, facade members, storage keys, and visible behavior.
...
2. Continue provider-internal cleanup after Slice 6: move the next small state domain, likely conversation write/persistence orchestration or local simulated-message helpers, out of `ProfileProvider` behind clearer stores/services without changing visible UI.
