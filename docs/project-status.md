...
- Local match activation and stale-match cleanup now live behind `expo/services/local-match-action-service.ts`. `ProfileProvider` still decides when a backend swipe is allowed to become a local visible match, and delegates local match removal for unmatch and backend chat `match_not_found` cleanup.
...
- Backend swipe persistence calls now live behind `expo/services/backend-swipe-action-service.ts`. `ProfileProvider` still owns local fixture activation, match-limit checks, and the decision to expose a visible match only after a backend reciprocal match result.
...
- Backend display-profile completeness, selection, and remembered-cache merge helpers now live behind `expo/services/backend-profile-display-service.ts`, shared by profile-provider lookup/cache hydration and backend match hydration planning. `ProfileProvider` still owns the refs, React state, and cache persistence calls.
...
2. Continue provider-internal cleanup after Slice 6/Slice 5b: isolate remaining backend-first action and profile lookup cache ownership paths without changing visible UI.
