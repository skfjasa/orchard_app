...
- Session-close handoff records the UAT tunnel, navigation back, forgot-password recovery checkpoint, and provider extraction checkpoint. Latest implementation checkpoint: pending commit - Discover/Fruit read model extracted.

## Current Task

Foundation Slice 6 read-path migration now covers Matches, Inbox, Match Detail, Chat, Discover, and Fruit. Discover and Fruit use focused read-model hooks while preserving existing deck/grid UI, discovery query behavior, remembered display-profile behavior, swipe/like actions, and local fixture separation.
...
2. Continue foundation Slice 6 with Profile/safety/paywall/onboarding provider calls, without changing visible UI or auth/profile behavior.
