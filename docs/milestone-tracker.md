...
- [x] [C] Local seen-match state/ref application for the `markMatchSeen` facade now delegates to `expo/services/local-interaction-service.ts`.
...
- [x] [C] Local purchase/subscription result application now lives in `expo/services/local-monetization-service.ts`; `ProfileProvider` keeps only the compatibility facade actions.
