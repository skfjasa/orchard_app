...
Slices 2 through 6, Slice 5b, and Part 4 are implemented. Provider-internal cleanup has moved selector, prototype monetization, local monetization result application, chat UI state, local conversation persistence, seen-match state application, local chat simulation timing, backend match lookup, backend swipe pass persistence, backend chat send/read action orchestration, Supabase discovery fixture filtering, React Query polling/realtime invalidation handover, and pure conversation helper extraction behind focused helpers/services. The remaining provider-owned paths are higher-risk because they touch bootstrap, chat fixture behavior, or source-of-truth transitions; choose the next PR-sized slice explicitly and pair nontrivial movement with targeted UAT.
